import { Task } from "@/app/types/Task";
import { QUADRANTS } from "@/app/types/Quadrant";

export const emptyTasks: Task[] = [];
export const onboardingTasks: Task[] = [
  {
    id: "1",
    title: "Prioritize this task",
    description:
      "This task is a placeholder to help you get started. It will be replaced with your own tasks soon.",
    quadrant: QUADRANTS[1],
    createdAt: new Date("2025-02-01"),
    completed: false,
    completedAt: undefined,
    status: "todo",
  },
];

export const tasks: Task[] = [
  {
    id: "1",
    title: "Create wireframes",
    description:
      "Design low-fidelity wireframes for the landing page and user dashboard.",
    dueDate: new Date("2025-02-05"),
    quadrant: QUADRANTS[0],
    createdAt: new Date("2025-02-01"),
    completed: false,
    completedAt: undefined,
    status: "todo",
  },
  {
    id: "2",
    title: "Define color palette",
    dueDate: new Date("2025-02-05"),
    quadrant: QUADRANTS[2],
    createdAt: new Date("2025-02-02"),
    completed: false,
    completedAt: undefined,
    status: "todo",
    projectId: "1",
  },
  {
    id: "3",
    title: "Develop interactive prototype",
    description: "Create a clickable prototype using Figma for user testing.",
    dueDate: new Date("2025-02-10"),
    quadrant: QUADRANTS[1],
    createdAt: new Date("2025-02-03"),
    updatedAt: new Date("2025-02-04"),
    completed: false,
    completedAt: undefined,
    status: "doing",
    projectId: "2",
  },
  {
    id: "4",
    title: "Conduct user interviews",
    description:
      "Interview 5 potential users to gather feedback on the wireframes.",
    dueDate: new Date("2025-02-08"),
    dueTime: new Date("2025-02-08T10:00:00"),
    quadrant: QUADRANTS[1],
    createdAt: new Date("2025-02-02"),
    completed: false,
    completedAt: undefined,
    status: "todo",
    projectId: "1",
  },
  {
    id: "5",
    title: "Implement design system",
    quadrant: QUADRANTS[3],
    createdAt: new Date("2025-02-01"),
    completed: false,
    completedAt: undefined,
    status: "todo",
    projectId: "2",
  },
  {
    id: "6",
    title: "Review accessibility compliance",
    description: "Ensure all UI elements meet WCAG 2.1 guidelines.",
    dueDate: new Date("2025-02-15"),
    quadrant: QUADRANTS[2],
    createdAt: new Date("2025-02-05"),
    completed: false,
    completedAt: undefined,
    status: "doing",
    projectId: "1",
  },
  {
    id: "7",
    title: "Optimize mobile responsiveness",
    quadrant: QUADRANTS[1],
    createdAt: new Date("2025-02-03"),
    updatedAt: new Date("2025-02-06"),
    completed: false,
    completedAt: undefined,
    status: "doing",
  },
  {
    id: "8",
    title: "Finalize typography choices",
    description:
      "Select heading, body, and button fonts for the design system.",
    dueDate: new Date("2025-02-07"),
    quadrant: QUADRANTS[2],
    createdAt: new Date("2025-02-02"),
    completed: true,
    completedAt: new Date("2025-02-07"),
    status: "done",
    projectId: "2",
  },
  {
    id: "9",
    title: "Prepare design handoff",
    description:
      "Export assets and document design decisions for the development team.",
    dueDate: new Date("2025-02-12"),
    quadrant: QUADRANTS[1],
    createdAt: new Date("2025-02-04"),
    updatedAt: new Date("2025-02-06"),
    completed: false,
    completedAt: undefined,
    status: "todo",
    projectId: "2",
  },
  {
    id: "10",
    title: "Refine microinteractions",
    description: "Add subtle animations to improve user experience.",
    dueDate: new Date("2025-02-13"),
    quadrant: QUADRANTS[3],
    createdAt: new Date("2025-02-05"),
    completed: false,
    completedAt: undefined,
    status: "todo",
  },
  {
    id: "11",
    title: "Set up design feedback sessions",
    description: "Schedule weekly reviews with stakeholders and developers.",
    dueDate: new Date("2025-02-14"),
    quadrant: QUADRANTS[2],
    createdAt: new Date("2025-02-06"),
    updatedAt: new Date("2025-02-07"),
    completed: false,
    completedAt: undefined,
    status: "todo",
    projectId: "2",
  },
  {
    id: "12",
    title: "Document UI design principles",
    description: "Create a reference guide for consistent design decisions.",
    quadrant: QUADRANTS[4],
    createdAt: new Date("2025-02-07"),
    completed: false,
    completedAt: undefined,
    status: "todo",
  },
  {
    id: "13",
    title: "Update design tokens",
    description:
      "Ensure global styles are applied correctly across components.",
    dueDate: new Date("2025-02-10"),
    quadrant: QUADRANTS[1],
    createdAt: new Date("2025-02-06"),
    completed: false,
    completedAt: undefined,
    status: "doing",
  },
  {
    id: "14",
    title: "Research latest UI trends",
    description:
      "Investigate emerging design patterns for modern web applications.",
    dueDate: new Date("2025-02-05"),
    quadrant: QUADRANTS[3],
    createdAt: new Date("2025-02-05"),
    completed: true,
    completedAt: new Date("2025-02-09"),
    status: "done",
  },
  {
    id: "15",
    title: "Prepare stakeholder presentation",
    description: "Summarize key design decisions for the next review meeting.",
    dueDate: new Date("2025-02-18"),
    quadrant: QUADRANTS[0],
    createdAt: new Date("2025-02-08"),
    updatedAt: new Date("2025-02-09"),
    completed: false,
    completedAt: undefined,
    status: "todo",
  },
  {
    id: "16",
    title: "Create user onboarding flow",
    description:
      "Design and implement an intuitive onboarding experience for new users",
    dueDate: new Date("2025-02-20"),
    quadrant: QUADRANTS[1],
    createdAt: new Date("2025-02-10"),
    completed: false,
    completedAt: undefined,
    status: "todo",
    projectId: "1",
  },
  {
    id: "17",
    title: "Optimize image assets",
    description:
      "Compress and optimize all image assets for better performance",
    dueDate: new Date("2025-02-22"),
    quadrant: QUADRANTS[0],
    createdAt: new Date("2025-02-10"),
    completed: false,
    completedAt: undefined,
    status: "todo",
  },
  {
    id: "18",
    title: "Set up analytics tracking",
    description:
      "Implement Google Analytics and set up key conversion tracking",
    dueDate: new Date("2025-02-25"),
    quadrant: QUADRANTS[0],
    createdAt: new Date("2025-02-11"),
    completed: false,
    completedAt: undefined,
    status: "doing",
    projectId: "2",
  },
  {
    id: "19",
    title: "Create email templates",
    description: "Design and code responsive email templates for notifications",
    dueDate: new Date("2025-02-23"),
    quadrant: QUADRANTS[2],
    createdAt: new Date("2025-02-11"),
    completed: false,
    completedAt: undefined,
    status: "todo",
  },
  {
    id: "20",
    title: "Write API documentation",
    description: "Document all API endpoints and usage examples",
    dueDate: new Date("2025-02-28"),
    quadrant: QUADRANTS[3],
    createdAt: new Date("2025-02-12"),
    completed: false,
    completedAt: undefined,
    status: "todo",
    projectId: "1",
  },
  {
    id: "21",
    title: "Fix critical security vulnerability",
    description:
      "Address recently discovered authentication bypass in login flow",
    dueDate: new Date("2025-02-15"),
    quadrant: QUADRANTS[0],
    createdAt: new Date("2025-02-13"),
    completed: false,
    completedAt: undefined,
    status: "todo",
    projectId: "1",
  },
  {
    id: "22",
    title: "Deploy emergency hotfix",
    description: "Push critical bug fixes to production environment",
    dueDate: new Date("2025-02-14"),
    quadrant: QUADRANTS[0],
    createdAt: new Date("2025-02-13"),
    completed: false,
    completedAt: undefined,
    status: "doing",
    projectId: "2",
  },
  {
    id: "23",
    title: "Resolve database performance issue",
    description:
      "Investigate and fix database bottleneck causing system slowdown",
    dueDate: new Date("2025-02-16"),
    quadrant: QUADRANTS[0],
    createdAt: new Date("2025-02-13"),
    completed: false,
    completedAt: undefined,
    status: "todo",
  },
  {
    id: "24",
    title: "Prepare quarterly board presentation",
    description: "Create and finalize presentation for emergency board meeting",
    dueDate: new Date("2025-02-17"),
    dueTime: new Date("2025-02-17T09:00:00"),
    quadrant: QUADRANTS[0],
    createdAt: new Date("2025-02-13"),
    completed: false,
    completedAt: undefined,
    status: "todo",
    projectId: "1",
  },
  {
    id: "25",
    title: "Address client escalation",
    description: "Resolve urgent issues reported by major client",
    dueDate: new Date("2025-02-15"),
    quadrant: QUADRANTS[0],
    createdAt: new Date("2025-02-13"),
    completed: false,
    completedAt: undefined,
    status: "doing",
    projectId: "2",
  },
] as const;
