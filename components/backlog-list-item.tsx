...
export function BacklogListItem({ task }: { task: Task }) {
  return (
    <div className="group/list-item flex items-start gap-3.5 py-2.5 px-3.5 rounded-lg hover:bg-zinc-100/[.75] hover:cursor-grab transition-all duration-150">
      ...
      <div className="flex flex-col gap-1.5 peer-has-data-[state=checked]:line-through peer-has-data-[state=checked]:text-zinc-400">
        <p className="grow text-base leading-snug font-medium text-zinc-800">
          {task.title}
        </p>

        {task.dueDate && (
          <div className="flex items-center gap-1.5 text-xs font-semibold text-zinc-400 group-hover/list-item:text-zinc-500 font-mono">
            {task.dueDate}
          </div>
        )}
      </div>
